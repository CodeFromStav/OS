...
int getStringLength( char *testStr )
    {
        ...
        while( counterIndex < MAX_STR_LEN && testStr[ counterIndex ] != NULL_CHAR )
            {
...
Precondition: given C-style source string, having a null character ('\0')
                at end of string; destination string is passed in as a 
...
Algorithm: compares two string alphabetically such that:
            ...
            if oneStr > otherStr, the function returns a value >0
            if oneStr == otherStr, the function return a 0
            if two strings are identical up to the point that one is longer,
                the difference  in length will be returned
...
int compareString ( char *oneStr, char *otherStr )
    {
        ...
        while( index < MAX_STR_LEN 
            ...
            {
            //find difference between the currently aligned characters
                    // a.k.a compare index 1 of StrOne to index 1 of StrTwo
        ...
        //assume string are equal at this point, return string length difference
            //function: getStringLength
...
void getSubString( char *destStr, char *sourceStr,
                                     int startIndex, int endIndex )  
    {
        ...
        //otherwise, assume indices not in limits      
            ...
            {
             //create empty string with null characters
             destStr[ 0 ] = NULL_CHAR;
            }
...
PreCondition: given a C-style test string, having a null character ('\0')
                at end of string, and given search string with a null
...
/*
Function Name: setChatToLowerCase
Algorithm: tests character parameter forupper case, changes it to lower case,
...
int getLineTo( FILE *filePtr, int bufferSize, char stopChar,
                            char *buffer, Boolean omitLeadingWhiteSpace )
    {
        ...
        while( charAsInt != (int) stopChar && bufferSizeAvailable == True )
            ...
            //check for useable (printable) character
...
PreCondition: file is oen with valid file pointer
